...
import { listCategoriesAdmin, createCategory, updateCategory, deleteCategory, CategoryPayload } from "endpoints";
import { toaster } from "components";
import { APICallOptions, Category } from "types";
...
export const useAdminCategories = () => {
    ...
    const [editingItem, setEditingItem] = useState<Category | "new" | null>(null);
    ...
    const saveItem = async (payload: CategoryPayload) => {
        if (isSaving) return;
        setIsSaving(true);
        setFormErrors([]);
        ...
        try {
            ...
            const apiErrors = error?.response?.data?.body?.category?.error;
            setFormErrors(
    ...
    const requestDelete = (item: Category) => setDeletingItem(item);
    const cancelDelete = () => {
